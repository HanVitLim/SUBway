...
const TTSButton = ({ text, transdata }) => {
  ...
  return(
    <div>
      <button type="button" onClick={() => handleTTS(text)}>{text} 읽기</button>
      <button type="button" onClick={() => handleTTS_kor(transdata)}>{transdata} 읽기</button>
    </div>
  )
};
